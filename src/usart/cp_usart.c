...
volatile uint8_t g_v_puc_linebuffer[10];
...
void concat_dma_buf_cb(uint8_t *buff0, uint8_t *buff1, int len_b0, int len_b1) {
    ...
    if (count == sizeof(g_v_puc_linebuffer)) {
        memcpy(linebuffer_data, g_v_puc_linebuffer, sizeof(g_v_puc_linebuffer));
        tail = 0;
        count = 0;
    }
}
...
void USART_Handler(void) {
    ...
    if (ul_status & US_CSR_RXBUFF) {
        ...
        concat_dma_buf_cb(gs_puc_buffer[gs_uc_buf_num], gs_puc_nextbuffer[gs_uc_buf_num], gs_ul_size_buffer, gs_ul_size_nextbuffer);
...
int read(void * rx_buf, size_t size) {
    
    usart_getchar(BOARD_USART, rx_buf);
    // char * buf_ptr = rx_buf;
    // int c = 0;
    // size_t nb_done;
    // size_t s;
    // if (size) {
    //     printf("Size checked out: %d\n\r", size);
    //     while (nb_done < size) {
    //         printf("nb done < n_elem\n\r");
    //         s = size;
    //         do {
    //             if (c = getc(rx_buf) != '\n\r') {
    //                 printf("parsing for null end of buf\n\r");
    //                 *buf_ptr++ = c;
    //             } else {
    //                 printf("done\n\r");
    //                 return nb_done;
